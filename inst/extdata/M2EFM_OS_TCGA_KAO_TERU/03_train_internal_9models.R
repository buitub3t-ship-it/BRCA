...
compute_or_load_rors <- function(tcga_exp, tcga_clin) {
  if (file.exists(RORS_RDS)) {
    ...
    score <- as_numeric_safe(score)
    names(score) <- names(readRDS(RORS_RDS))
  ...
  pam50_packages <- c("M2EFM", "genefu", "org.Hs.eg.db")
  missing_pam50 <- pam50_packages[
    !vapply(pam50_packages, requireNamespace, logical(1), quietly = TRUE)
  ]

  if (length(missing_pam50) > 0L) {
    stop(
      "Cannot build rorS and rorS+Clin because no rorS column was found ",
      "and these packages are missing: ",
      paste(missing_pam50, collapse = ", "),
      "\nInstall them, restart R, and rerun this script.",
      "\nBiocManager::install(c(\"genefu\", \"org.Hs.eg.db\"))",
      "\nInstall the local M2EFM package from the repository root if needed."
    )
  }

  message("Computing PAM50 rorS with M2EFM::get_pam50() ...")

  pam50 <- M2EFM::get_pam50(
    as.data.frame(tcga_exp)
  )

  risk_table <- pam50$pam50risk

  if (is.null(risk_table) || ncol(risk_table) < 1L) {
    stop("M2EFM::get_pam50() did not return pam50risk.")
  }

  score <- as_numeric_safe(risk_table[[1L]])
  names(score) <- gsub(
    "\\.",
    "-",
    rownames(risk_table)
...
if (
  ...
  if (is.data.frame(partial) || data.table::is.data.table(partial)) {
    ...
    message(
      "Resuming comparison models from: ",
      PARTIAL_RDS
    )
  }
}

completed_splits <- comparison_metrics[
  model %in% c(
    "Cox Meth+Exp",
    "Cox Meth+Exp+Clin",
    "rorS",
...
for (split_id in seq_len(N_MONTE_CARLO_SPLITS)) {
  if (split_id %in% completed_splits) {
    message(
  ...
  split_rows <- tryCatch(
    {
      ...
      rors_train <- rors[train_ids]
      rors_test <- rors[test_ids]
      ...
      data.table::data.table(
        split = rep(split_id, 4L),
        cohort = rep("TCGA_test", 4L),
        model = c(
          "Cox Meth+Exp",
          "Cox Meth+Exp+Clin",
          "rorS",
  ...
  comparison_metrics <- data.table::rbindlist(
    list(comparison_metrics, split_rows),
    use.names = TRUE,
    fill = TRUE
  )
